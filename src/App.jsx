...
function App() {
  ...
  const fetchExpirations = async (tickerSymbol) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/expirations?ticker=${encodeURIComponent(tickerSymbol.toUpperCase())}`)
      
      // Check if response is actually JSON
      const contentType = response.headers.get('content-type')
      if (!contentType || !contentType.includes('application/json')) {
        const text = await response.text()
        throw new Error(`Server returned non-JSON response. Make sure backend is running on port 5050. Response: ${text.substring(0, 100)}`)
      }
      ...
      if (err.message.includes('fetch')) {
        setError('Cannot connect to backend. Make sure the backend server is running on port 5050.')
      } else {
  ...
  const handleCalculate = async (e) => {
    ...
    try {
      ...
      if (!contentType || !contentType.includes('application/json')) {
        const text = await response.text()
        throw new Error(`Server returned non-JSON response. Make sure backend is running. Response: ${text.substring(0, 100)}`)
      }
  ...
  const buildIvSurface = async () => {
    ...
    try {
      ...
      for (const exp of selectedExpirations) {
        ...
        const data = await response.json()
        if (!response.ok) {
          throw new Error(data.error || `Failed to fetch option data for expiration ${exp}`)
        }
    ...
    <div className="app">
      ...
      <main className="main-content">
        <form onSubmit={handleCalculate} className="options-form">
          ...
          <button type="submit" className="submit-button" disabled={loading || loadingExpirations}>
          ...
          <div className="results-container">
            <div className="results-header">
              ...
              <div className="results-summary">
                <div className="summary-item">
                  ...
                  <span className="summary-value">{formatCurrency(results.currentPrice)}</span>
                </div>
                <div className="summary-item">
                  <span className="summary-label">Time to Maturity:</span>
                  <span className="summary-value">{formatNumber(results.timeToMaturity)} years</span>
                </div>
                <div className="summary-item">
                  <span className="summary-label">Risk-Free Rate:</span>
                  <span className="summary-value">{formatPercent(results.riskFreeRate * 100)}</span>
                </div>
                <div className="summary-item">
                ...
                <div className="summary-item">
                  ...
                  <span className="summary-value">{results.optionType.toUpperCase()}</span>
                </div>
              </div>
            </div>
                ...
                <tbody>
                  {results.results.map((row, index) => (
                    <tr key={index}>
                      ...
                      <td className="option-value">{formatCurrency(row['American Option Value'])}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {expirations.length > 1 && (
              <div className="iv-surface-controls">
                ...
                {surfaceError && <div className="error-message iv-surface-error">{surfaceError}</div>}
              </div>
            )}
